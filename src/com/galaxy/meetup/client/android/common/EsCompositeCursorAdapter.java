 ...
 */
package com.galaxy.meetup.client.android.common;
...
public abstract class EsCompositeCursorAdapter extends BaseAdapter {
	...
	public EsCompositeCursorAdapter(Context context) {
		this(context, INITIAL_CAPACITY);
	}

	public EsCompositeCursorAdapter(Context context, int initialCapacity) {
		mContext = context;
	...
	public final int getCount(int i) {
        ensureCacheValid();
        int j;
        if(mPartitions[i].cursor == null)
            j = 0;
        else
            j = mPartitions[i].cursor.getCount();
        return j;
    }
	...
	public final void checkPartitions(String s, String s1) {
        int i = -1 + mSize;
        while(i >= 0)  {
            Partition partition = mPartitions[i];
            Cursor cursor;
            if(partition != null)
                ...
                cursor = null;
            if(cursor == null)
                Log.i(s, (new StringBuilder("partcheck s:")).append(s1).append(" emptypart:").append(i).toString());
            else
            if(cursor.isClosed())
                Log.i(s, (new StringBuilder("partcheck s:")).append(s1).append(" stalepart:").append(i).toString());
            i--;
        }
    }
